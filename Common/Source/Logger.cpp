...
namespace CB{
	...
	namespace Log{
		...
		void	CLogger::AddStream(CRefPtr<IO::IStream> pStream){
			CRefPtr<IEntryFormatter> pFormatter = Log::CTextFormatter::Create().Cast<Log::IEntryFormatter>();
			this->AddStream(pStream, pFormatter);
		}

		void	CLogger::AddStream(CRefPtr<IO::IStream> pStream, CRefPtr<Log::IEntryFormatter> pFormatter){
